...
const SUBSYSTEM: &str = "fish.doom.apple-log.store-tests";

fn unique_category(label: &str) -> String {
...
fn wait_for_entries(
    ...
    loop {
        let entries = store
            .get_entries(OSLogEnumeratorOptions::NONE, None, filter, 1_000)
            .expect("entries");
...
fn log_entries(entries: Vec<OSLogStoreEntry>) -> Vec<OSLogEntryLog> {
    entries
        .into_iter()
